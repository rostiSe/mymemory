...
import { extractContentFromUrl } from '../tools/extract-content';
import {
  extractMediumArticleWithFirecrawl,
  isMediumArticleUrl,
} from '../tools/extract-content-medium-firecrawl';
import { summarizeText } from '../tools/summarize';
import { generateEmbedding } from '../tools/generate-embedding';
import { generateTags } from '../tools/generate-tags';
import { extractTopics } from '../tools/extract-topics';
import { assignSpace } from '../tools/assign-space';
import { findRelatedEntries } from '../tools/find-related-entries';
...
export async function processEntry(entryId: string, userId: string) {
  try {
    ...
    const { assignSpace } = await import('../tools/assign-space');
    const { findRelatedEntries } = await import('../tools/find-related-entries');
